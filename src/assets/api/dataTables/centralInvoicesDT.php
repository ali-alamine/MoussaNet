...
$rowsCount = mysqli_fetch_assoc(mysqli_query(openConn(), "SELECT COUNT(INVCID) as exp FROM invoice_central"))['exp'];
...
if (isset($_GET["search"]["value"]) && !empty($_GET["search"]["value"])) {
    ...
    $getAllFactureQuery = "select * from invoice_central " . $orderString . " LIMIT " . $rowsReq . " OFFSET " . $start;
...
if ($getAllFactureQuerySQL) {
    while ($row = mysqli_fetch_assoc($getAllFactureQuerySQL)) {
        if ($row != null) {
            ...
            $jsonData = $jsonData . '"duration":"' . $row['duration'] . '",';
            $jsonData = $jsonData . '"ID":"' . $row['INVCID'] . '",';
            $jsonData = $jsonData . '"country":"' . $row['country'] . '",';
