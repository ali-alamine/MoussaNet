...
if (isset($_GET["search"]["value"]) && !empty($_GET["search"]["value"])) {
    ...
    $getAllFactureQuery = "select invoice.date,invoice.INVID,invoice.IID,invoice.quantity,invoice.profit,item.name,invoice.price,person.name as pName from invoice inner join person on invoice.PID = person.PID inner join item on invoice.IID = item.IID where invoice.type IN ('RC','CT','OF')   " . $orderString . " LIMIT " . $rowsReq . " OFFSET " . $start;
