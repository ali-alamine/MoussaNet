...
$condition = "where DATE(oper_date) = DATE(NOW())";

if (isset($_GET['date'])  && $_GET['date'] != "") {
    $searchDate = $_GET['date'];
    $condition = "where DATE(oper_date) = '".$searchDate."'";
}

if (isset($_GET['paidFlag'])  &&  $_GET['paidFlag'] == "1") {    
    $condition = "where oper_is_paid = 0 ";
}

$rowsCount = mysqli_fetch_assoc(mysqli_query(openConn(), "SELECT COUNT(*) as exp FROM omt_operation ".$condition))['exp'];
...
$getAllFactureQuery = "select * , DATE_FORMAT(oper_date,'%H:%i:%s') as oper_time from omt_operation inner join person on omt_operation.oper_client_id = person.PID " .$condition ." ". $orderString . " LIMIT " . $rowsReq . " OFFSET " . $start;
...
if ($getAllFactureQuerySQL) {
    while ($row = mysqli_fetch_assoc($getAllFactureQuerySQL)) {
        if ($row != null) {
            ...
            $jsonData = $jsonData . '"oper_time":"' . $row['oper_date'] . '",';
