...
export class SubscribersReportComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    subscriberDataTable.on('select', function (e, dt, type, indexes) {
      ...
      if (type === 'row') {
        SubscribersReportComponent.selectedRowData = subscriberDataTable.row(indexes).data();
        var data = subscriberDataTable.row(indexes).data()['ID'];
        SubscribersReportComponent.selectedSubscriberID = data;
      }
      else if (type === 'column') {
        SubscribersReportComponent.selectedSubscriberID = -1;
      }
    });
  ...
  togglePayment() {
    console.log(SubscribersReportComponent.selectedRowData);
    this.subscriberReportService.togglePayment(SubscribersReportComponent.selectedRowData['subDetID']).subscribe(Response => {
  ...
  deleteSubscription() {
    this.subscriberReportService.deleteSubscription(SubscribersReportComponent.selectedRowData['subDetID']).subscribe(Response => {
      this.globalSubscriberReportDT.ajax.reload(null, false);      
    }, error => {
      console.log(error);
    });
